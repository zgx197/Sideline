# Lattice 底座模块文档路线图（Phase D0 - D8 已完成）
...
| P2 | Entity / Frame / Storage | `ECS/FrameStorageDesign.md` | 已完成 | 这是整个 ECS 状态容器的核心 |
| P3 | Query / Filter / BlockIterator | `ECS/QueryDesign.md` | 已完成 | 这是系统读取数据的正式入口 |
| P4 | CommandBuffer / 结构修改闭环 | `ECS/CommandBufferDesign.md` | 已完成 | 结构修改正式闭环已经写清，系统与 session 才能继续收敛 |
| P5 | System 基座 | `ECS/SystemDesign.md` | 已完成 | 系统生命周期、调度与观察边界已正式收敛 |
| P6 | 最小 SimulationSession 闭环 | `ECS/SimulationSessionDesign.md` | 已完成 | verified / predicted 最小闭环与 playback 时机已正式定义 |
| P7 | Snapshot / Checksum / Input / Rollback 约束 | `ECS/DeterministicLoopDesign.md` | 已完成 | 最小确定性循环与当前 resim 边界已正式定义 |
| P8 | Runtime Bridge / Runner / Debug Tool | `ECS/RuntimeLayerDesign.md` | 已完成 | 上层运行时边界已收敛，但仍保持在底座主线之外 |
...
### Phase D6：SimulationSession 文档（已完成）
...
### Phase D8：上层运行时文档（已完成）

**目标**

在底座主线稳定后，正式定义以下上层运行时能力的边界：
...
- 已新增 `ECS/RuntimeLayerDesign.md`
- 已明确这些能力建立在 `SimulationSession` 之上
- 已明确它们不属于底座主线、但属于当前整体架构的下一层
