...
从当前阶段开始，Facet 的编辑器 / 运行时职责划分、工作台入口和运行时调试边界，也统一维护在本文档中，不再单独维护额外的重构计划文档。
...
Facet 在当前代码库中已经明确分成两条链路：

- 编辑器链路：`godot/addons/facet-tools`
- 运行时链路：`godot/scripts/facet`

这两条链路的分工已经冻结，不再把验证期的临时工具继续混在一起。

编辑器侧的 `Facet Main Panel` 现在只承担：
...
运行时侧的 `Facet Runtime Debug` 负责承接：

- 运行时热重载测试
- 页面切换与状态观察
- Projection / Binding / RedDot 诊断
- 运行日志与快照验证

这意味着两条明确约束：

- 编辑器插件不再直接承担运行时实验台职责
- 运行时调试能力不依赖编辑器主面板是否打开

当前阶段仍然不拆新的 `Facet.Editor` 或 `Facet.Runtime.Debug` 独立 `csproj`。原因不是边界不清楚，而是边界已经先通过目录、职责和文档冻结，程序集拆分留到后续确有必要时再做。
