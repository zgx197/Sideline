...
> 本文档用于整理 Sideline 当时的 CI 优化基线、优先级与实施顺序。当前仓库中的 Godot 自动发布已经在此规划之外以独立 workflow 方式落地，因此本文中的相关 P2 结论应按“历史收口决策”理解，而不是当前未实现状态。
...
- Godot 自动发布流程已按当时决策从 CI 优化主线中剥离，并已在后续独立发布议题中单独实现。
...
| P2 | 将 Godot 自动发布流程转入独立议题 | 避免未定方案污染当时的 CI 结构 | 已作为独立发布轨道推进 | 已完成 |
